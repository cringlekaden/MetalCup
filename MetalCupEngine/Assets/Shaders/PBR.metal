...
public:
    ...
    static float GeometrySmithDirect(float3 N, float3 V, float3 L, float roughness) {
        float NdotV = max(dot(N, V), 0.0);
        float NdotL = max(dot(N, L), 0.0);
        float ggxV = GeometrySchlickGGXDirect(NdotV, roughness);
        float ggxL = GeometrySchlickGGXDirect(NdotL, roughness);
        return ggxV * ggxL;
    }
    ...
    static float GeometrySchlickGGX(float NdotV, float roughness) {
        return GeometrySchlickGGXDirect(NdotV, roughness);
    }

    static float GeometrySmith(float3 N, float3 V, float3 L, float roughness) {
        return GeometrySmithDirect(N, V, L, roughness);
    }
    ...
    static float2 integrateBRDF(float NdotV, float roughness, uint sampleCount) {
        ...
        for(uint i = 0; i < sampleCount; ++i) {
            ...
            if(NdotL > 0.0) {
                ...
                float denom = max(NdotH * NdotV, 1e-5);
